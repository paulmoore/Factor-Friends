...
function public.byteParity(byte)
    byte = bit.bxor(byte, bit.brshift(byte, 4));
    byte = bit.bxor(byte, bit.brshift(byte, 2));
    byte = bit.bxor(byte, bit.brshift(byte, 1));
...
function public.getByte(number, index)
    if (index == 0) then
        ...
        return bit.band(bit.brshift(number, index*8),0xff);
    end
end
